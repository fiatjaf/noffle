...
export default {
    subscribe,
    updateFollow: (action, pubKey) => update((state) => {
        if (action === "follow") {
    ...
    initStore: (data) => {
        update((state) => {
            ...
            state.home = new SortedMap(notes.map((n) => [n.id + ":" + n.created_at, n]), (a, b) => b.split(":")[1] - a.split(":")[1]);
            state.pubKeyHex = getPublicKey(state.key);
            return state;
        });
    },
    ...
    receivedTextNote: (evt, mine = false) => {
        ...
        update((state) => {
            ...
            if (evt.ref && evt.ref.length) {
                state.browsing.set(`rel:${evt.ref.slice(0, 5)}:${evt.id.slice(0, 5)}`, evt);
            }
            return state;
        });
    },
    receivedMetadata: (pubkey, meta) => {
        update((state) => {
            state.metadata.set(pubkey, meta);
            return state;
        });
    },
};